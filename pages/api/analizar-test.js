...
export default async function handler(req, res) {
  ...
  // 🔐 Token interno (lo envía el worker /api/worker/analizar)
  const expected = (process.env.AUREA_INTERNAL_TOKEN || "").trim();
  const got = String(req.headers["x-internal-token"] || "").trim();

  if (!expected || got !== expected) {
    // Log mínimo para diagnóstico (sin exponer el token completo)
  ...
  // 🔗 ENDPOINTS (tus GAS + mailer en Vercel)
  ...
  try {
    // ⚠️ Importante: necesitamos mínimo tipoInstitucion para GAS_RESP_URL
    // Si el body viene vacío por alguna prueba, esto fallará con 400.
...
`.trim();
    ...
    // 4) OpenAI con validación y reintento corto
    ...
    // 5) Enviar correo (Usuario + SOS + Alfredo) con reintento corto
    const destinatarios = [
      correoUsuario,
      (correoSOS || "").trim(),
